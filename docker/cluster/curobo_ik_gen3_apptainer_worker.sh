#!/usr/bin/env bash
# Apptainer + partial IsaacLab binds: cuRobo IK demo (Isaac Sim python + GPU).
# Invoked by srun_curobo_ik_gen3_model.sh on a GPU node.
set -euo pipefail
...
echo "[$(date -Is)] IK demo REPO_ROOT=${REPO_ROOT} COBOT2_ROOT=${COBOT2_ROOT} SIF=${ISAACLAB_SIF}" >&2
test -f "${ISAACLAB_SIF}"

IK_INNER='cd /workspace/isaaclab && exec "${ISAACSIM_PATH}/python.sh" scripts/tutorials/00_sim/curobo_inverse_kinematics_gen3_demo.py'
if [[ "$#" -eq 0 ]]; then
  apptainer_cmd=(bash -lc "${IK_INNER}")
else
  QUOTE_ARGS=$(printf '%q ' "$@")
  apptainer_cmd=(bash -lc "${IK_INNER} ${QUOTE_ARGS}")
fi
