#!/usr/bin/env bash
# Shared host steps for cuRobo Gen3 YAML build (Apptainer + partial IsaacLab binds).
# Invoked by build_curobo_gen3_model.slurm or srun_build_curobo_gen3_model.sh on a GPU node.
set -euo pipefail
...
# After module load: Lmod `module purge` can clear SCRATCH if sourced earlier.
...
echo "[$(date -Is)] REPO_ROOT=${REPO_ROOT} COBOT2_ROOT=${COBOT2_ROOT} SIF=${ISAACLAB_SIF} SCRATCH=${SCRATCH}" >&2
test -f "${ISAACLAB_SIF}"

# Do not pass --env HOME (Apptainer warns when it conflicts with APPTAINERENV_* on some sites).
# Isaac Sim in this SIF runs as root; map host user-site into /root/.local.
apptainer exec --nv --cleanenv \
  ...
  "${ISAACLAB_SIF}" bash -lc 'cd /workspace/isaaclab && bash scripts/tutorials/00_sim/build_curobo_gen3_model.sh'
